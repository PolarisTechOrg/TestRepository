//
//  Twelve_XianPanZaiDieStrategy.m
//  OptionWuKong
//
...
@implementation Twelve_XianPanZaiDieStrategy
...
- (BOOL)Verify:(CombineStrategyOrder*) strategyOrder Error:(out NSString *) errorMessage
{
    ...
    if (leftOrder.OrderSide != FcpOrderSide_Sell || leftOption.OptionType != FcpOptionsType_Put)
    {
        errorMessage = @"左侧只能卖出看跌期权";
        return NO;
    }
    if (rightOrder.OrderSide != FcpOrderSide_Buy || rightOption.OptionType != FcpOptionsType_Put)
    {
        errorMessage = @"右侧只能买入看跌期权";
        return NO;
    }
    ...
    {
        errorMessage = @"买入看跌期权行权价应等于卖出看跌期权行权价";
        return NO;
    }
...
- (OptionStrategyDetail *)InternalCalcOptionStrategyDetail:(CombineStrategyOrder *)strategyOrder
{
    ...
    Twelve_XianPanZaiDieProfitPoint *profitPoint = [[Twelve_XianPanZaiDieProfitPoint alloc] init];
