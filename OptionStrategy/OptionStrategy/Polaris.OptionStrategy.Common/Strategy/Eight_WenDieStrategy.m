...
@implementation Eight_WenDieStrategy
...
- (BOOL)Verify:(CombineStrategyOrder*) strategyOrder Error:(out NSString *) errorMessage
{
    ...
    {
        errorMessage = @"卖出看跌期权行权价应小于买入看跌期权行权价";
        return NO;
    }
...
- (OptionStrategyDetail *)InternalCalcOptionStrategyDetail:(CombineStrategyOrder *)strategyOrder
{
    ...
    double oneMaxProfit = (leftOption.StrikePrice - rightOption.StrikePrice - diffPrice) * volumeMultiple - oneFeeLeft - oneFeeRight;   // 最大盈利
    ...
    Eight_WenDieProfitPoint *profitPoint = [[Eight_WenDieProfitPoint alloc] init];
    ...
    NSMutableArray *itemList = [NSMutableArray arrayWithCapacity:ARRAY_CAPACITY];
    {
        ...
        item.SeqNum = 1;
        item.Title = @"最大可能获利：";
        item.Content = [NSString stringWithFormat:@"%@元，结算指数(价)在%@点以下", [self DoubleRound:oneMaxProfit * orderQty], [self DoubleRound:profitPoint.StrikePoint1]];
    ...
    }
    {
        ...
        item.SeqNum = 2;
        item.Title = @"最大可能亏损：";
        item.Content = [NSString stringWithFormat:@"%@元，结算指数(价)在%@点以上", [self DoubleRound:oneMaxLoss * orderQty], [self DoubleRound:profitPoint.StrikePoint2]];
        [itemList addObject:item];
    }
    {
        OptionStrategyDetailItem *item = [[OptionStrategyDetailItem alloc] init];
