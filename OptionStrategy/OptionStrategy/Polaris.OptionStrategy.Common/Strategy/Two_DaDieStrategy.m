...
@implementation Two_DaDieStrategy
...
- (BOOL)Verify:(CombineStrategyOrder*) strategyOrder Error:(out NSString *) errorMessage
{
    ...
    if (option.OptionType != FcpOptionsType_Put || order.OrderSide != FcpOrderSide_Buy)
    {
        errorMessage = @"只能买入看跌期权";
        return NO;
    }
...
- (OptionStrategyDetail *)InternalCalcOptionStrategyDetail:(CombineStrategyOrder *)strategyOrder
{
    ...
    double equalPoint = option.StrikePrice - (oneAmount / volumeMultiple); // 损益两平点
    double totalAmount = oneAmount * order.OrderQty;
    
    Two_DaDieProfitPoint *profitPoint = [[Two_DaDieProfitPoint alloc] init];
    ...
    NSMutableArray *itemList = [NSMutableArray arrayWithCapacity:ARRAY_CAPACITY];
    {
        ...
        item.SeqNum = 1;
        item.Title = @"最大可能获利：";
        item.Content = [NSString stringWithFormat:@"结算价小于%@点以上的无限获利空间", [self DoubleRound:profitPoint.EqualPoint]];
