...
@implementation Four_NiuPiPianDanStrategy
...
- (BOOL)Verify:(CombineStrategyOrder*) strategyOrder Error:(out NSString *) errorMessage
{
    ...
    if (option.OptionType != FcpOptionsType_Call || order.OrderSide != FcpOrderSide_Sell)
    {
        errorMessage = @"只能卖出看涨期权";
        return NO;
    }
...
- (OptionStrategyDetail *)InternalCalcOptionStrategyDetail:(CombineStrategyOrder *)strategyOrder
{
    ...
    Four_NiuPiPianDanProfitPoint *profitPoint = [[Four_NiuPiPianDanProfitPoint alloc] init];
    ...
    }
    {
        ...
        item.SeqNum = 2;
        item.Title = @"最大可能亏损：";
        item.Content = [NSString stringWithFormat:@"在%@点以上风险无限", [self DoubleRound:profitPoint.EqualPoint]];
        [itemList addObject:item];
    }
    {
        OptionStrategyDetailItem *item = [[OptionStrategyDetailItem alloc] init];
